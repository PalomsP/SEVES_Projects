...
def interpol_2(X, Z, R, grid_X, grid_Z, method='linear'):
    """
    Prends les données de ton modres.dat qui ont été extraite du dataframe sous frome de vecteur
    et elle réalise l'interpolation linéaire sur un maillage défini par grid_x et grid_z'
...
def prof_horizons(dt_tableau) :
    ...
    avec d'abord le recouvrement, puis l'horizon altéré, puis
...
def log_et_panneau(X_interpol, Z_interpol, R_interpol, logf, dt_tableau, oid):
    ...
    ylim = 76 # Permet de définir la profondeur maximale de la représentation
